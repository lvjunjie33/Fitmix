 ...
 */
@MsgSubscribeAnnotation(channel = MsgConstants.CHANNEL_TYPE_OTHER_LINK)
public class OtherLinkPushCommand extends PushModule implements RedisConcurrentlyCommand {
    ...
    public void execute(String msgId) {
        ...
        if (StringUtils.isEmpty(targetUid)) {
            pushAllHandle(title, content, body);
        } else {
            //查询设备信息
            ...
            pushHandle(active, terminal, deviceToken, title, content, body);
        }
    }

    public Map<String, String> formatMap(Map<String, Object> map) {
        Map<String, String> msg = new HashMap<>();
        msg.put("msgBody", JSON.toJSONString(map));
        return msg;
    }
}
