...
<div class="uk-container-center uk-body">
    <div class="uk-form-row">
        <form action="/ugc/albums-list.htm" method="get" class="uk-form">
            <div class="uk-form-row">
                <div class="uk-form-row">
                    <label>专辑ID：</label>
...
<div class="uk-overflow-container">
    <table class="uk-table uk-table-striped uk-text-nowrap uk-table-striped uk-table uk-table-hover">
        <thead>
        <tr>
            <th style="width: 100px;">专辑编号</th>
            <th style="width: 200px;">专辑名称</th>
            <th style="width: 200px;">专辑简介</th>
        ...
        <tbody>
        <c:forEach var="albums" items="${page.result}">
            <tr>
                <td><label name="id">${albums.id}</label></td>
                <td>${albums.title}</td>
                <td>${albums.desc} </td>
                <td><lch:parse-date time="${albums.addTime}" pattern="yyyy-MM-dd HH:mm"/></td>
                <td>
                    <c:choose>
                        <c:when test="${albums.checkStatus == 0}">
                            待审核
                        </c:when>
                        <c:when test="${albums.checkStatus == 1}">
                            审核通过
                        </c:when>
                        <c:when test="${albums.checkStatus == 2}">
                            审核未通过
                        </c:when>
                    </c:choose>
                </td>
                <td>
                    <lch:auth-operation url="/ugc/check-albums.htm">
                        <a href="/ugc/check-albums.htm?id=${albums.id}" target="_blank">审核</a>
                    </lch:auth-operation>
                    &nbsp;
...
<%--分页条--%>
<lch:page page="${page}" href="/ugc/albums-list.htm"></lch:page>
<script>
    $(function(){
