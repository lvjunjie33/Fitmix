...
//                    .append(BASE_36_CHAR[MathUtil.nextInt(BASE_36_CHAR.length)]);
    ...
    public static Set<String> getLocalIps() {
        try {
            ...
            while(enumerations.hasMoreElements()) {
                Enumeration<InetAddress> addrs = enumerations.nextElement().getInetAddresses();
                while (addrs.hasMoreElements()) {
                    String addr = addrs.nextElement().getHostAddress();
                    if (StringUtils.countOccurrencesOf(addr, ".") != 3) {
                        continue;
                    }
                    ips.add(addr);
                }
    ...
    public static void downloadFile (HttpServletResponse response, InputStream is, String fileName, String fileSuffix) {
        response.setContentType("application/x-msexcel; charset=UTF-8");
        response.setCharacterEncoding("UTF-8");
        OutputStream os = null;
        try {
            response.setHeader("Content-Disposition", "attachment; filename=\""
                    + new String((fileName + "." + fileSuffix).getBytes("GBK"), "iso8859-1")
                    + "\"");
            os = response.getOutputStream();
            byte[] buffer = new byte[1024];
            int z = 0;
            while ((z = is.read(buffer)) != -1) {
                os.write(buffer);
            }
            os.flush();
        } catch (IOException e) {
