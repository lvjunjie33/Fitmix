$(window).load(function () {
    ...
    (function() {
        // 步幅
        if($('#bufu').length > 0) {
            var bufu = new CountUp("bufu", 0, (window.share.attributes.distance / window.share.attributes.step).toFixed(2), 2, durationTime, {
                useEasing: true,
                easingFn: easingFn,
                useGrouping: false,
                separator: '',
    ...
    if($("#ranzhi").length > 0){
        if(typeof window.share.attributes.calorie != 'undefined' && window.share.attributes.calorie != null) {
            // 步数
            var ranzhi = new CountUp("ranzhi", 0, Math.floor(window.share.attributes.calorie / 10), 0, durationTime, {
                useEasing: true,
                easingFn: easingFn,
                useGrouping: false,
                separator: '',
