...
//        userRun.setMark(userRunMark(calBpmMatch));
    ...
    public List<UserRun>  historyUserRunLimit(String cityId) {
        ...
        for(User user:listUser){
            ...
            if(listUserRun.size()>0){
                for(UserRun userRun:listUserRun){
                    userRun.setGw_avatar(user.getAvatar());
                    userRun.setGw_name(user.getName());
                }
    ...
    public synchronized int addWatchRun (UserRun userRun) {
        ...
        //手表运动记录存在且状态为0“失效”
        if(beforeUserRun != null && beforeUserRun.getState()==0){
            //将原有运动记录改成1“有效”
            userRunDao.updateUserRunState(beforeUserRun.getUid(), userRun.getStartTime(), Update.update("state", UserRun.STATE_EFFECTIVE).set("updateTime", System.currentTimeMillis()));
        }
        //手表运动记录不存在
        if(beforeUserRun==null){
            //插入
     ...
     * @param uid 测试手表文件
     */
    public String getZipFile (Integer uid) {
        UserRun userRun = userRunDao.findLastUpdateUserRun(uid);
        return userRun.getWatchSourceZipFile();
    }
