package com.business.app.easeMob.comm;
...
public class ClientContext {
	 ...
	 * Properties
	 */
	private static final String API_PROTOCAL_KEY = "HX_API_PROTOCAL";
	
	private static final String API_HOST_KEY = "HX_API_HOST";
	
	private static final String API_ORG_KEY = "HX_API_ORG";
	
	private static final String API_APP_KEY = "HX_API_APP";
	
	private static final String APP_CLIENT_ID_KEY = "HX_APP_CLIENT_ID";
	
	private static final String APP_CLIENT_SECRET_KEY = "HX_APP_CLIENT_SECRET";
	
	private static final String APP_IMP_LIB_KEY = "HX_APP_IMP_LIB";
	...
	private void initFromPropertiesFile() {
		...
		try {
			InputStream inputStream = ClientContext.class.getClassLoader().getResourceAsStream("hx-config.properties");
			p.load(inputStream);
		} catch (IOException e) {
			log.error(MessageTemplate.print(MessageTemplate.FILE_ACCESS_MSG, new String[]{"hx-config.properties"}));
		...
		if( StringUtils.isBlank(protocal) || StringUtils.isBlank(host) || StringUtils.isBlank(org) || StringUtils.isBlank(app) || StringUtils.isBlank(clientId) || StringUtils.isBlank(clientSecret) || StringUtils.isBlank(impLib) ) {
			log.error(MessageTemplate.print(MessageTemplate.INVAILID_PROPERTIES_MSG, new String[]{"hx-config.properties"}));
			return; // Context not initialized
		}
