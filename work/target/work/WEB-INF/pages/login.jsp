<%--
  Created by IntelliJ IDEA.
  User: sin
  Date: 15/3/30
  Time: 下午7:57
  To change this template use File | Settings | File Templates.
--%>
<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<%@ include file="/WEB-INF/pages/common/head-uikit.jsp" %>
<style type="text/css">
    .uk-form>div{margin: 10px;}
    html,body{height: 100%;margin: 0px;padding: 0px;}
</style>
<script type="text/javascript">
    $(function(){
        $("html").keydown(function(e){
            if (e.keyCode == 13) {
                $(".login-form").submit();
            }
        });

        $("#dl-id").click(function() {
            $(".login-form").submit();
        });
    });
</script>

<body>
    <div class="uk-panel uk-panel-box uk-vertical-align" style="height: inherit;padding: 0px;margin: 0px;border: 0px;">
        <div class="uk-panel uk-text-center uk-vertical-align-middle uk-container-center uk-width-2-1" style="margin-top: -80px">
            <h3>Admin Name</h3>
            <img class="uk-border-circle" width="120" height="120" src="data:image/svg+xml;base64," alt="">
            <form class="uk-form login-form" method="post" action="login.json" isform>
                <div>
                    <div class="uk-form-password">
                        <input type="text" name="username" placeholder="mobile" class="uk-form-width-medium" notnull>
                    </div>
                </div>
                <div>
                    <div class="uk-form-password">
                        <input type="password" name="password" placeholder="password" class="uk-form-width-medium" notnull>
                        <a href="javascript:;" class="uk-form-password-toggle" id = "dl-id">登录</a>
                    </div>
                </div>
            </form>
        </div>
    </div>
</body>


